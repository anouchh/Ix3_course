...
def compute_quality_flags(
    ...
    if flags.get("has_constant_columns", False):
        score -= 0.15
    
    if flags.get("has_suspicious_id_duplicates", False):
        score -= 0.25

    score = max(0.0, min(1.0, score))
    flags["quality_score"] = score

    # Ищем колонки, где все значения одинаковы (или все NaN)
    constant_columns = []
    for col_name in df.columns:
        unique_non_null = df[col_name].dropna().unique()  # Уникальные не-NaN значения
        if len(unique_non_null) <= 1:  # 0 (все NaN) или 1 (все одинаковые)
            constant_columns.append(col_name)
    
    flags["has_constant_columns"] = len(constant_columns) > 0
    flags["constant_columns_list"] = constant_columns  # Список проблемных колонок
    flags["constant_columns_count"] = len(constant_columns)

    # Ищем колонки, которые могут быть ID (по имени и типу)
    suspicious_duplicates = {}
    
    possible_id_columns = [
        col for col in df.columns 
        if "id" in col.lower() or "uuid" in col.lower() or "guid" in col.lower()
    ]

    for col in possible_id_columns:
        # Пропускаем колонки с почти все NaN
        non_null_count = df[col].notna().sum()
        if non_null_count == 0:
            continue  # Все значения NaN
        ...
        if unique_count < total_rows:
            ...
            duplicate_share = duplicate_count / total_rows
            
            suspicious_duplicates[col] = {
                "duplicate_count": duplicate_count,
                "duplicate_share": duplicate_share,
                "unique_count": unique_count
            }
    
    flags["has_suspicious_id_duplicates"] = len(suspicious_duplicates) > 0
    flags["suspicious_duplicates_dict"] = suspicious_duplicates
