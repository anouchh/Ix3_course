...
class QualityFlagsResponse(BaseModel):
    """Ответ с полным набором булевых флагов качества."""
    
    flags: Dict[str, bool] = Field(
        ...,
        description="Полный набор булевых флагов качества"
    )
    quality_score: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Общая оценка качества данных (0..1)"
    )
    latency_ms: float = Field(
        ...,
        ge=0.0,
...
@app.post(
    ...
    tags=["quality"],
    summary="Полный набор булевых флагов качества из CSV-файла",
    description="""Принимает CSV-файл, анализирует его и возвращает полный набор
    булевых флагов качества, включая все эвристики из HW03.""",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> Dict[str, Any]:
    """
    Возвращает полный набор булевых флагов качества для CSV-файла.
    """
    
    start = perf_counter()
    
    # Проверка типа файла (как в quality-from-csv)
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(
            status_code=400, 
    ...
    for key, value in flags_all.items():
        # Пропускаем числовые поля, списки и словари
        if key in ["quality_score", "max_missing_share", 
                   "constant_columns_count", "constant_columns_list",
                   "suspicious_duplicates_dict"]:
            continue
        
        # Добавляем только булевы флаги
        if isinstance(value, bool):
            flags_bool[key] = value
    
    latency_ms = (perf_counter() - start) * 1000.0
    
    # Логирование для отладки (как в других эндпоинтах)
    print(
        f"[quality-flags-from-csv] filename={file.filename!r} "
        f"n_rows={df.shape[0]} n_cols={df.shape[1]} "
        f"flags_count={len(flags_bool)} latency_ms={latency_ms:.1f} ms"
    )
    
    # Возвращаем ТОЛЬКО словарь с флагами, как требуется в задании
    return {
        "flags": flags_bool
    }