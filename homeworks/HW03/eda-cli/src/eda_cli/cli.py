...
def report(
    ...
    # 3. Колонки с критическими пропусками
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Постоянные колонки: **{quality_flags.get('has_constant_columns', 'N/A')}**")
        if quality_flags.get('has_constant_columns', False):
            count = quality_flags.get('constant_columns_count', 0)
            f.write(f" ({count} колонок)")
        f.write("\n")
        
        f.write(f"- Дубликаты в ID: **{quality_flags.get('has_suspicious_id_duplicates', 'N/A')}**")
        if quality_flags.get('has_suspicious_id_duplicates', False):
            dup_dict = quality_flags.get('suspicious_duplicates_dict', {})
            if dup_dict:
                f.write(f" (проблемные колонки: {', '.join(dup_dict.keys())})")
        f.write("\n\n")

        if problem_missing_cols:
            f.write(f"\n### Колонки с высоким процентом пропусков (> {min_missing_share:.0%})\n\n")
...
def head(
    ...
    n: int = typer.Option(
        5, 
        help="Количество первых строк для вывода.",
        min=1
    ),
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
    ...
    Примеры:
        eda-cli head data.csv           # выведет первые 5 строк
        eda-cli head data.csv --n 10    # выведет первые 10 строк
        eda-cli head data.csv --n 3 --sep ";"  # с другим разделителем
    """
    df = _load_csv(Path(path), sep=sep, encoding=encoding)
    ...
    # Ограничиваем n размером датасета
    n_rows = min(n, len(df))
    
    typer.echo(f"Первые {n_rows} строк из {len(df)}:")
    
    # Выводим DataFrame с красивым форматированием
    typer.echo(df.head(n_rows).to_string())
